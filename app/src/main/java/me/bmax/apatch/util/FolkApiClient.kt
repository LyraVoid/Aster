...
object FolkApiClient {
    ...
    /** A request the server refused; retrying it would only repeat the same answer. */
    private class HttpFailure(code: Int) : IOException("HTTP $code")
    ...
    suspend fun fetchJson(
        ...
        // Nothing but http(s) is worth handing to the network stack, so other schemes are dropped.
    ...
    private suspend fun fetchWithRetry(url: String, maxRetries: Int): String {
        ...
        repeat(maxRetries + 1) { attempt ->
            try {
                ...
                Log.w(TAG, "Refused by $url: ${e.message}")
                throw e
    ...
    fun prefetch(vararg urls: String) {
        urls.forEach { url ->
            scope.launch {
                val result = fetchJson(url, forceRefresh = false)
                Log.d(TAG, "Prefetch $url: ${if (result.isSuccess) "done" else "failed"}")
            }
        }
    }
}
