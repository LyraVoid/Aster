...
pub fn refresh_ap_package_list(skey: &CStr, mutex: &Arc<Mutex<()>>) {
    ...
    if let Err(error) =
        with_synchronized_package_config(|configs| apply_package_list(skey, configs))
    {
...
mod sync_tests {
    ...
    fn retains_granted_and_critical_uids_only() {
        ...
        assert_eq!(
            revoked_uids(&current, &[config.clone()]),
            vec![110123, 10456]
        );
